...
namespace mrsd
{
	...
	// add enemies with randomly, set up projectile with x,y force, vx, vy, turrentAngle, minAngle
	...
	// if the player is in the dangerZone at the time, 
	// it dies return false, else return true
	...
	// if the current explosion time > explosionTime, dangerZone[i] --, return false, else return true
	...
	// if the y axis is below 0, explode and return false, else true
	...
	{
		time += timestep;
		// std::cout << "Game Time Step: " << time << std::endl;
	...
	// if trajectiles explode, dangerZone[i] will +1, after the explosion, dangerZone[i] -1
